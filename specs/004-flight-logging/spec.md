...
- **Lifecycle / Offline Behavior**: Manual entries, optional manual flight locations, saved imported flights, edits, removals, GPS tracks, telemetry series, and derived usage relationships must remain available offline, after backgrounding, and after relaunch. Interrupted manual entry, location selection, import review, edit, or removal flows must either complete safely or leave the last saved Logbook state intact.
...
- Flight Logging is scoped to the current local pilot; multi-pilot sharing and cross-device sync are out of scope for this feature.
